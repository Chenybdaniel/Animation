...
class MyHomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final animationWrapper = _AnimationWrapper(
        GlobalKey<_AnimationWrapperState>(), Alignment.topCenter);
    ...
    var btn = RaisedButton.icon(
      ...
      label: Padding(
        ...
        child: Text('落下', style: TextStyle(fontSize: 18, color: Colors.white),),
      ),
      color: Colors.lightBlue,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
      onPressed: () {
        animationWrapper.setAlignment(Alignment.bottomCenter);
      },
    );
  ...
  Widget build(BuildContext context) {
    var w = Expanded(
      child: AnimatedContainer(
        ...
        child: Icon(Icons.ac_unit, color: Colors.lightBlue, size: 50),
        alignment: widget._alignment,
        onEnd: () {
          setState(() {
            widget._alignment = Alignment.topCenter;
          });
        },
      ),
    );
